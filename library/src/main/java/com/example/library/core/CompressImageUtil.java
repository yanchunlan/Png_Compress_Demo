...
public class CompressImageUtil {
    ...
    private void compressImageByQuality(final Bitmap bitmap, final String imgPath, final CompressResultListener listener) {
        if (bitmap == null || bitmap.isRecycled()) {
            sendMsg(false, imgPath, "质量压缩失败，bitmap为null", listener);
            return;
        }
    ...
    private void sendMsg(final boolean isSuccess, final String imagePath,
                         final String msg, final CompressResultListener listener) {
        handler.post(new Runnable() {
            ...
            public void run() {
                if (isSuccess) {
                    ...
                    listener.onCompressFailed(imagePath, msg);
                }
            }
        });
    }
